    ...
    <div class="row">
        <div class="col-12">
            <div class="card">
                <div class="card-body">
                    <div class="row">
                        <form action="/edit-book/{{ $book->slug }}" method="post" enctype="multipart/form-data" class="col-4 col-sm-12">
                            @csrf
                            @method('put')
                            <div class="mb-3">
                                <label for="example-text-input" class="form-label">Book Code</label>
                                <input class="form-control @error('book_code') is-invalid @enderror" name="book_code" type="text" placeholder="Enter Book Code" value="{{ $book->book_code }}" required disabled>
                                @error('book_code')
                            ...
                            <div class="mb-3">
                                ...
                                <input class="form-control @error('title') is-invalid @enderror" value="{{ $book->title  }}" name="title" type="text" placeholder="Enter Book Title" required >
                                @error('title')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-control" name="categories[]" value="{{ old('categories.0', '') }}" data-trigger name="choices-multiple-default" id="choices-multiple-default" placeholder="Choose Categories" multiple>
                                  ...
                                  <option value="{{ $category->id }}" {{ in_array($category->id, (array) old('categories')) ? 'selected' : '' }}>{{ $category->name }}</option>
                                  @endforeach
                                </select>
                              </div>
